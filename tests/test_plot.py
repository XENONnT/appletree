...
def test_regbin_edges_centers():
    ...
    # Linear: lower/upper are first/last node centers
    edges, centers = _regbin_edges_centers(0.0, 3.0, 3, is_log=False)
    np.testing.assert_allclose(centers, [0.0, 1.5, 3.0])
    np.testing.assert_allclose(edges, [-0.75, 0.75, 2.25, 3.75])

    # Log: lower/upper are first/last node centers
    ...
    np.testing.assert_allclose(
        edges,
        [10**-0.75, 10**0.75, 10**2.25, 10**3.75],
    )
...
def test_plot_helpers():
    ...
    map_2d = SimpleNamespace(
        ...
        coordinate_name=["x", "y"],
        coordinate_lowers=[1.0, 1.0],
        coordinate_uppers=[100.0, 100.0],
        map=np.ones((3, 3)),
    )
