...
class DECRecombination(Plugin):
    depends_on = ["num_quanta_x", "num_ion_x", "recomb_x", "num_quanta_y", "num_ion_y", "recomb_y"]
    provides = ["num_photon", "num_electron"]

    @partial(jit, static_argnums=(0,))
    def simulate(self, key, parameters, num_quanta_x, num_ion_x, recomb_x, num_quanta_y, num_ion_y, recomb_y):
        recomb = recomb_x * (1.0 - recomb_y) + recomb_y * (1.0 - recomb_x)
        renormalization = recomb + (1.0 - recomb_x) * (1.0 - recomb_y)

        p_not_recomb = 1.0 - recomb / renormalization
