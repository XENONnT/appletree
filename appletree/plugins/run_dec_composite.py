...
from appletree.plugins import const_dec_composite
...
@export
class CombinedRecomb(const_dec_composite.DECRecombComposeConstantR):
...
def running_r_recomb(n1, n2, s1, s2):
    ...
    def recomb(i, state):
        ...
        ra = jnp.where(c * a > 0, 1 - jnp.log1p(c * a) / (c * a), (1/2.)*c*a - (1/3.) * (c*a)**2)
        rb = jnp.where(d * b > 0, 1 - jnp.log1p(d * b) / (d * b), (1/2.)*d*b - (1/3.) * (d*b)**2)
        ra_ = ra * (1 - rb)
        rb_ = rb * (1 - ra)
...
# Added function for scaling factor
@jit
...
# Changed from using map to using the above function
@export
#@takes_config(
#    Map(
