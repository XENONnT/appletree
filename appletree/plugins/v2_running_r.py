...
from appletree.plugins import constant_r
...
class ECEnergy(Plugin):
    ...
    def simulate(self, key, parameters, batch_size):
        energy_x = jnp.full(batch_size, self.Auger.value)
        energy_y = jnp.full(batch_size, self.L1.value)
        energy_z = jnp.full(batch_size, self.L2.value)
...
@export
class CombinedRecomb(constant_r.RecombComposeConstantR):
    depends_on = ["recomb_x", "recomb_y", "recomb_z"]
    provides = ["recomb_const"]


@jit
def running_r_recomb(n1, n2, n3, s):
    a, b, c, d, e, f = n1, n2, n3, s/n1, s/n2, s/n3
...
class RecombComposeRunningR(Plugin):
    depends_on = ["recomb_const", "num_ion_x", "num_ion_y", "num_ion_z"]
    provides = ["recomb"]

    @partial(jit, static_argnums=(0,))
    def simulate(self, key, parameters, recomb_const, num_ion_x, num_ion_y, num_ion_z):
        #scale_x = 10 ** self.running_r_scaling.apply(recomb_x)
        #scale_y = 10 ** self.running_r_scaling.apply(recomb_y)
        scale = 10 ** running_r_scaling(recomb_const)
        recomb = v_running_r_recomb(
            num_ion_x, num_ion_y, num_ion_z, scale
        )
